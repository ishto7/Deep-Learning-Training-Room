...
def generate_batch(batch_size,skip_window):
    ...
    for i in range(batch_size):
        for j in range(skip_window*2):
            if j<skip_window:
                batch[i,j]=buffer[target-skip_window+j]
            elif j>=skip_window:
                batch[i,j]=buffer[target-skip_window+j+1]
            # [buffer[target-skip_window::target-1],buffer[target+1::target+skip_window]]
...
for num_skips, skip_window in [(2, 2), (4, 2)]:
    data_index = 0
    batch, labels = generate_batch(batch_size=8, skip_window=skip_window)
    print('\nwith num_skips = %d and skip_window = %d:' % (num_skips, skip_window))
    print('    batch:', [[reverese_dictionary[w] for w in bi] for bi in batch])
